...
Two_D_phi_L2_CvM_test<- function(X, Y, m, n, d, kernel=2){
  ...
  rho<- function(x, y, kernel){
    sum<- abs(x-y)^2
    return( (1/(d-1))*sum(phi( sum[-d]+sum[-1], kernel))  )
  }
  ...
  h <- function(Xi, Xj, Yk){
    return( acos((rho(Xi,Yk, kernel)+rho(Xj,Yk, kernel)-rho(Xi,Xj, kernel) )/
                   (2*sqrt(rho(Xi, Yk, kernel)*rho(Xj, Yk, kernel)))  )  )
  }
